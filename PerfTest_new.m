...
for alpha = [1 2]
    ...
    fprintf(fileid, 'size,TMFG_Val,TMFG_elapsed,TMFGT1_Val,TMFGT1_elapsed,TMFGT2_K4_Val,TMFGT2_K4_elapsed,TMFGL, TMFGL_Elapsed,PMFG_Val,PMFG_elapsed,UpperBound\n');
    for iter = 1:samples % e.g. 100 simulations
        for n = sizes % e.g. 400x400 matrices
            ...
            tic;[TL, t, tc ] = TMFGLoz(r,0); elapsed = toc;
            fprintf(fileid, '%f,%f,', sum(TL(:))+0.0, elapsed);
...
for alpha = [1]
    ...
    fprintf(fileid, 'Pareto Alpha %d\n', alpha);
    fprintf(fileid, 'size,TMFG_Val,TMFG_elapsed,TMFGT1_Val,TMFGT1_elapsed,TMFGT2_K4_Val,TMFGT2_K4_elapsed,TMFGL, TMFGL_Elapsed,PMFG_Val,PMFG_elapsed,UpperBound\n');
        for n = [50 100 150 300 500 700 850 1000 1200] % e.g. 400x400 matrices
            x = randraw('pareto', [1, alpha], [1 n*(n-1)/2]);
            x = (x - min(x)) ./ (max(x) - min(x));
            W = triu(ones(n,n),1);
            W(W==1) = x;
